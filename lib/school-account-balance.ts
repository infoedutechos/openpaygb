...
export async function getStudentTermPaidUgx(input: {
  organizationId: string;
  studentId: string;
  term: number;
}): Promise<number> {
  ...
  const allocatedPaid = await getAllocatedPaidUgx(input);
  if (allocatedPaid > 0) return allocatedPaid;
  ...
  const paymentTotal = paid._sum.totalUgx ?? 0;
  if (expected <= 0) return paymentTotal;
  return Math.min(expected, paymentTotal);
}
