...
export async function buildSchoolDashboard(organizationId: string, termInput: number): Promise<SchoolDashboardPayload> {
  const term = normalizeSchoolTerm(termInput);

  const [charges, confirmedPayments, outflows, salaryPaid, students, staff, inventory, defaulterAll] =
    await Promise.all([
      prisma.studentBillCharge.aggregate({
      ...
      listSchoolDefaulters({ organizationId, term }),
    ]);

  const expectedUgx = charges._sum.amountUgx ?? 0;
  const receivedUgx = confirmedPayments._sum.totalUgx ?? 0;
  const outstandingUgx = Math.max(0, expectedUgx - receivedUgx);
  ...
  return {
    ...
    cashflows: { incomeUgx: receivedUgx, expenditureUgx, netUgx: receivedUgx - expenditureUgx },
