...
function scoreArticle(
  ...
  for (const token of tokens) {
    if (title.includes(token)) score += relaxed ? 8 : 12;
    if (category.includes(token)) score += relaxed ? 4 : 6;
    if (tags.some((t) => t.includes(token))) score += relaxed ? 3 : 5;
    if (summary.includes(token)) score += relaxed ? 2 : 4;
    const bodyMatches = body.split(token).length - 1;
    score += Math.min(bodyMatches, relaxed ? 4 : 8);
    if (relaxed && token.length >= 3 && title.includes(token)) score += 2;
  }
  return score;
}

/** Looser match — used before auto-learning synthesizes an article. */
export async function searchKnowledgeBaseRelaxed(opts: {
  query: string;
  hub?: PlatformHub;
  limit?: number;
  ...
  for (const row of rows) {
    const score = scoreArticle(tokens, row, true);
    if (score < 2) continue;
    hits.push({
      id: row.id,
      slug: row.slug,
