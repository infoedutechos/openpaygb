...
export interface InitialGameState {
  ...
  /** Last time an API call confirmed the app server responded (incl. 4xx). Updated from sync, pearls/me, /api/health. */
  lastServerConfirmedAt: number | null;
  /** Last time a request indicated server/DB unreachable (network error or 5xx/503 health). */
  lastServerFailedAt: number | null;
}
...
export interface GameState extends InitialGameState {
  ...
  setTotalDonatedPoints: (points: number) => void;
  setFrozenState: (isFrozen: boolean, suspensionReason: string | null) => void;
  recordServerConfirmed: () => void;
  recordServerFailed: () => void;
}
...
export const calculateLevelIndex = (points: number, _totalTaps?: number): number => {
  for (let i = LEVELS.length - 1; i >= 0; i--) {
    ...
    if (points >= level.minPoints) return i;
  }
  return 0;
}
...
export const createGameStore = (initialState: InitialGameState) => create<GameState>((set) => ({
  ...
  setPoints: (points) => set((state) => {
    const newLevelIndex = calculateLevelIndex(points, state.totalTaps);
    return { points, gameLevelIndex: newLevelIndex };
  }),
  setTotalTaps: (totalTaps) => set((state) => {
    const newLevelIndex = calculateLevelIndex(state.points, totalTaps);
    return { totalTaps, gameLevelIndex: newLevelIndex };
  }),
  ...
  setTotalDonatedPoints: (totalDonatedPoints) => set({ totalDonatedPoints }),
  setFrozenState: (isFrozen, suspensionReason) => set({ isFrozen, suspensionReason }),
