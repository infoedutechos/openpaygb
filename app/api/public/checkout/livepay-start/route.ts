...
const Body = z
  .object({
    ...
    phone: E164,
    network: z.enum(["mtn", "airtel"]).transform((s) => s.toUpperCase() as LivePayNetwork),
  })
  .superRefine((val, ctx) => {
...
export async function POST(req: Request) {
  try {
    ...
    const reference = livePayCustomerReference(doc.id);

    let collect;
    try {
      collect = await livePayCollectMoney({
        phoneNumber: d.phone,
      ...
      const userMsg = livePayUserMessage(e);
      const r = resolveApiError(new Error(userMsg), {
        route: "checkout/livepay-start/collect",
        fallback: "Could not start LivePay collection",
      });
      const status = notConfigured
        ? 503
        : e instanceof LivePayApiError && e.httpStatus >= 400 && e.httpStatus < 500
          ? e.httpStatus
          : r.status < 500
            ? r.status
            : 502;
      if (r.shouldLog) console.error("[checkout/livepay-start/collect]", e);
      const code = notConfigured
        ? "livepay_not_configured"
        : /not allowed|allowlist/i.test(userMsg)
          ? "livepay_ip_not_allowed"
          : "livepay_collect_failed";
    ...
    const res = NextResponse.json(
      {
        ...
        livepay: {
          ...
          network: collect.network ?? d.network,
        },
      },
      { status: 201 },
    );
    ...
    return apiErrorResponse(e, {
      route: "checkout/livepay-start",
      fallback: "Could not start LivePay payment",
    });
  }
}
