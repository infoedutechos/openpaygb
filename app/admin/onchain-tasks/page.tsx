...
import OnchainTaskCard, { type ExtendedOnchainTask } from '@/components/OnchainTaskCard';
...
export default function AdminOnchainTasks() {
    ...
    const [onchainTasks, setOnchainTasks] = useState<ExtendedOnchainTask[]>([]);
    const [editingTask, setEditingTask] = useState<ExtendedOnchainTask | null>(null);
    ...
    function isExtendedOnchainTask(task: unknown): task is ExtendedOnchainTask {
        if (!task || typeof task !== 'object') return false;
        const t = task as Record<string, unknown>;
        return (
            typeof t.id === 'string' &&
            typeof t.smartContractAddress === 'string' &&
            typeof t.points === 'number' &&
            typeof t.price === 'string' &&
            typeof t.isActive === 'boolean' &&
            (t.collectionMetadata === null || typeof t.collectionMetadata === 'object') &&
            (t.itemMetadata === null || typeof t.itemMetadata === 'object')
        );
    }

    const handleEdit = (task: ExtendedOnchainTask) => {
        setEditingTask(task);
        reset({
