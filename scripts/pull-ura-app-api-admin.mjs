/**
 * Pulls `app/api/admin/**` from ura-pearl-data-center (excluding Pay `summary` route).
 * Usage: node scripts/pull-ura-app-api-admin.mjs
 *
 * GitHub Contents API is rate-limited for unauthenticated requests. Set GITHUB_TOKEN
 * (classic PAT, no scopes required for public repo) to avoid HTTP 403 mid-sync.
 */
...
const REF = "main";
const PREFIX = "app/api/admin";

const ROOT = path.resolve(path.join(path.dirname(fileURLToPath(import.meta.url)), ".."));
const TARGET = path.join(ROOT, PREFIX);

const SKIP_PREFIXES = [`${PREFIX}/summary`];
...
console.error("Done. Kept Pay-only route: app/api/admin/summary/route.ts");
