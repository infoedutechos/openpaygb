...
See **`docs/FOLDER_STRUCTURE.md`** and **`npm run migrate:ura-layout`** (or robocopy flow in **FOLDER_STRUCTURE.md**) if an old clone still has `src/app` / `src/lib`.
...
Regenerate a pruned ASCII tree anytime: **`npm run docs:tree:write`** → `docs/FOLDER_TREE_SNAPSHOT.txt`.
