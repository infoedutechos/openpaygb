...
The **`migrate:*`** / **`fix:team-join-duplicates`** scripts are **data/maintenance tools** that assume the generated client matches `prisma/schema.prisma`. Older notes in `docs/architecture/FOLDER_STRUCTURE.md` mention **game-era** workflows that may require swapping in `schema.game.prisma` before running those tools.
...
- `docs/architecture/FOLDER_STRUCTURE.md` — overview of `verify`, `db:push`, `db:generate`, `build`.
- `docs/flows/PLATFORM_FLOWS.md` — mentions `npx prisma generate` and `npx prisma db push`.
