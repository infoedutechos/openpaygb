...
After **`npm run seed`** (+ optional **`npm run seed:uwais`**), defaults are:

| What | Where |
|------|--------|
| School checkout | `/pay/riverside-demo` |
| School admin | `school.admin@odelhub.local` → `/admin/login?school=1` · password `ChangeMe_Admin123!` |
| School student | `school.student@odelhub.local` → `/student/login` (slug `riverside-demo`) · `ChangeMe_Student123!` |
| University student | `student@odelhub.local` → `/student/login` (slug `default`) · `ChangeMe_Student123!` |
| University org admin | `admin@odelhub.local` → `/school/login` · `ChangeMe_Admin123!` |
| Platform master | `master@odelhub.local` → `/admin/login?master=1` · `ChangeMe_Master123!` |
| **Uwais** admin | `uwais.admin@odelhub.local` → `/admin/login?school=1` · `ChangeMe_Admin123!` · slug `uwais` · checkout `/pay/uwais` |
| Parent portal | `/parent` (School Pay Code + admission no. — no password) |
| Developers | `/developers/register` (self-serve; no seeded app) |
| OPGB console (master) | `/admin/master/opgb-ops` |
...
| Uwais checkout | http://localhost:3000/pay/uwais |
| Parent portal | http://localhost:3000/parent |
| Developers | http://localhost:3000/developers |
| OPGB master console | http://localhost:3000/admin/master/opgb-ops |
...
## 0b. Uwais pilot (after `npm run seed:uwais`)
...
| Fee ledger | http://localhost:3000/admin/fee-ledger |
| Sample CSV | `data/uwais-fee-ledger-sample.csv` |
| Parent lookup | http://localhost:3000/parent |

Roadmap: **[UWAIS_SMIS_PRIORITY_ROADMAP.md](./UWAIS_SMIS_PRIORITY_ROADMAP.md)**.

---

## 0c. OpenPayGB payment provider (developers)

| Step | URL / action |
|------|----------------|
| Register app | http://localhost:3000/developers/register |
| Dashboard | http://localhost:3000/developers/dashboard |
| Provider lobby | http://localhost:3000/opgb |
| Master console | http://localhost:3000/admin/master/opgb-ops (master login) |
| Docs | [OPENPAYGB_PAYMENT_PROVIDER.md](./OPENPAYGB_PAYMENT_PROVIDER.md) · [PLATFORM_UPDATE_2026-09.md](./PLATFORM_UPDATE_2026-09.md) |

Create Partner API key with `charges:create`, `charges:read`, `payouts:create`, `payouts:read`. Sandbox checkout when LivePay unset.
...
See **[LEDGER_RECEIPTS_AND_SCHOOL_UNITS.md](../school/LEDGER_RECEIPTS_AND_SCHOOL_UNITS.md)** for column definitions and code paths.
...
Full lifecycle: **[ORGANIZATION_REGISTRATION.md](../school/ORGANIZATION_REGISTRATION.md)** · sign-in detail: **[SCHOOL_ADMIN_LOGIN.md](../school/SCHOOL_ADMIN_LOGIN.md)**.
...
| Variable | Purpose |
|----------|---------|
| `SEED_ADMIN_EMAIL` / `SEED_ADMIN_PASSWORD` | School admin seed (default `admin@odelhub.local` / `ChangeMe_Admin123!`) |
| `SEED_MASTER_EMAIL` / `SEED_MASTER_PASSWORD` | Master seed (default `master@odelhub.local` / `ChangeMe_Master123!`) |
| `SEED_STUDENT_EMAIL` / `SEED_STUDENT_PASSWORD` | Demo student (`ChangeMe_Student123!`) |
| `SEED_SCHOOL_ADMIN_EMAIL` | Riverside admin (default `school.admin@odelhub.local`) |
| `SEED_SCHOOL_STUDENT_EMAIL` | Riverside student |
| `SEED_UWAIS_ADMIN_EMAIL` / `SEED_UWAIS_ADMIN_PASSWORD` | Uwais admin (`uwais.admin@odelhub.local`) |
...
| `OPENPAYGB_CHARGES_SANDBOX` | Force sandbox confirm on merchant checkout |
...
**VixonPay + virtual card (MoMo top-up & tuition):** full setup, webhooks, tunnels, and troubleshooting — **[VIXONPAY_VIRTUAL_CARD_AND_DEV.md](../deployment/VIXONPAY_VIRTUAL_CARD_AND_DEV.md)**.
...
| Bot + webhook + menu setup | See **[TELEGRAM_MINI_APP.md](../deployment/TELEGRAM_MINI_APP.md)** — webhook/menu require **HTTPS** `NEXT_PUBLIC_APP_URL` (production or tunnel) |
...
- [PLATFORM_UPDATE_2026-09.md](./PLATFORM_UPDATE_2026-09.md) — **commands + all logins + OPGB/Uwais recipes (2026-09)**
- [OPENPAYGB_PAYMENT_PROVIDER.md](./OPENPAYGB_PAYMENT_PROVIDER.md) — merchant charges, fees, white-label, cashout
- [PARTNER_API.md](./PARTNER_API.md) — Partner API scopes
- [UWAIS_SMIS_PRIORITY_ROADMAP.md](./UWAIS_SMIS_PRIORITY_ROADMAP.md) — Uwais SMIS roadmap
...
- [TELEGRAM_MINI_APP.md](../deployment/TELEGRAM_MINI_APP.md) — bot landing, `/tma` screens, setup commands
- [APP_STATUS_AUDIT.md](../product/APP_STATUS_AUDIT.md) — holistic scan, gaps, deployment readiness
- [SCHOOL_ADMIN_PROGRAMMES.md](../school/SCHOOL_ADMIN_PROGRAMMES.md) — what school admins can customize
- [USER_FLOW.md](../flows/USER_FLOW.md) — guest pay APIs and receipts
